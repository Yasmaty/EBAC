...
def main():
    ...
    if (data_file_1 is not None):
        ...
        st.write('Há quantos dias o cliente fez a sua última compra?')
        ...
        st.write("Um jeito de segmentar os clientes é criando quartis para cada componente do RFV, sendo que o melhor quartil é chamado de 'A', o segundo melhor quartil de 'B', o terceiro melhor de 'C' e o pior de 'D'. O melhor e o pior dependem da componente. Por exemplo, quanto menor a _recência_, melhor é o cliente (pois ele comprou com a gente tem pouco tempo) logo o menor quartil seria classificado como 'A'; já pra componente _frêquencia_ a lógica se inverte, ou seja, quanto maior a frêquencia do cliente comprar com a gente, melhor ele/a é, logo, o maior quartil recebe a letra 'A'.")
        st.write('Se estivermos interessados em mais ou menos classes, basta aumentarmos ou diminuirmos o número de quartis pra cada componente.')
        ...
        st.write('### Agrupamento por K-means')
        st.markdown('''Dado que há 3 ações de marketing a serem utilizadas de acordo com o RFV do cliente,
                    serão feitos 3 grupos iniciais por K-means, e os resultados serão comparados com os 
                    obtidos anteriormente.''')
        
        # Padronização das variáveis numéricas do de_RFV
        padronizador = StandardScaler()

        # Variáveis numéricas do RFV
        RFV_num = df_RFV.select_dtypes(include='number')
        
        # Padroniza as variáveis RFV
        RFV_padronizado = pd.DataFrame(padronizador.fit_transform(RFV_num))

        # Prepara o K-Means
        kmeans = KMeans(n_clusters=3, random_state=15)

        # Ajusta o K-means no RFV_padronizado e imprime resultados
        kmeans.fit(RFV_padronizado)
        RFV_num['grupo'] = kmeans.labels_
        st.write("#### Gráficos das variáveis agrupadas")
        st.pyplot(sns.pairplot(RFV_num, hue='grupo'))
        st.write('#### Tabela de comparação dos resultados')
        df_RFV['grupo'] = kmeans.labels_
        st.write(pd.crosstab(df_RFV['grupo'], df_RFV['acoes de marketing/crm']))
